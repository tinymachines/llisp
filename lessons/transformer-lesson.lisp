...
(defun matmul-3d (a b)
  "Matrix multiplication for 3D tensors (batch, seq, dim) or 3D x 2D"
  (let* ((batch (array-dimension a 0))
         (seq (array-dimension a 1))
         (dim1 (array-dimension a 2))
         (dim2 (if (= (array-rank b) 2) 
                   (array-dimension b 1)
                   (array-dimension b 2)))
    ...
    (dotimes (batch-idx batch)
      (dotimes (s seq)
        (dotimes (d2 dim2)
          (let ((sum 0.0))
            (dotimes (d1 dim1)
              (incf sum (* (aref a batch-idx s d1) 
                           (if (= (array-rank b) 2)
                               (aref b d1 d2)
                               (aref b batch-idx s d1 d2)))))
            (setf (aref result batch-idx s d2) sum)))))
    result))
...
(defun create-ffn (embed-dim ff-dim)
...
(defun create-transformer-block (embed-dim num-heads ff-dim)
  "Create transformer block"
  (make-transformer-block
   :mha (make-mha num-heads embed-dim)
   :ffn (create-ffn embed-dim ff-dim)
  ...
  (let ((model (make-transformer
    ...
    (dotimes (i num-layers)
      (setf (aref (transformer-blocks model) i)
            (create-transformer-block embed-dim num-heads ff-dim)))
